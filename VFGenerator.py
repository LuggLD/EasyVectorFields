...
global maxbounds 
global filename 
global directionbias 
global directionstrength 
global scalenoiseamount 
global directionnoiseamount 
global mainscalefactor
global generator 
...
def writevector(file, vector):
    ...
    for c in range(0, 3):
        try:
            ...
            print "ERROR: trying to write invalid vector: %s" % vector  # TODO: Find out why this happens.
...
def makeVectorField(_gridsize,_minbounds,_maxbounds,_generator,_filename,_directionbias,_directionstrength,_scalenoiseamount,_directionnoiseamount,_mainscalefactor):
    global gridsize 
    global minbounds 
    global maxbounds 
    global filename 
    ...
    directionstrength = _directionstrength
    scalenoiseamount = _scalenoiseamount
    directionnoiseamount = _directionnoiseamount
    mainscalefactor = _mainscalefactor
    ...
    """
    test = [_gridsize,_minbounds,_maxbounds,_generator,_filename,_directionbias,_directionstrength,_scalenoiseamount,_directionnoiseamount,_mainscalefactor,bounds,vectors]
    print "Testing input: "
    print test
    """
    ...
    gen = generatortypes[generator] # Selected from dropdown menu in Tkinter
    ...
    f = open(filename, 'w')
    f.truncate()
    writevector(f, [gridsize[2], gridsize[1], gridsize[0]])
    writevector(f, [minbounds[2], minbounds[1], minbounds[0]])
    writevector(f, [maxbounds[2], maxbounds[1], maxbounds[0]])
